                ...
                <div class="card">
                    ...
                    <form class="needs-validation" novalidate action="{{ route($route . '.update', $row->id) }}"
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label for="crop">Select Crops <span>*</span></label>
                                <select class="form-control" name="crop_id" id="crop" required>
                                    <option value="">{{ __('dashboard.select') }}</option>
                                    @foreach ($crops as $crop)
                                        <option value="{{ $crop->id }}"
                                            @if ($crop->id == $row->service_id) selected @endif>{{ $crop->title }}
                                        </option>
                                    @endforeach
                                </select>

                                <div class="invalid-feedback">
                                    {{ __('dashboard.please_provide') }} {{ __('dashboard.crop') }}
                                </div>
                            </div>
